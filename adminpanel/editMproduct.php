...
if (isset($_GET['i'])) {
    ...
    $select = "SELECT mproduct.*, category_name, brand_name
FROM mproduct
INNER JOIN category
ON category.category_id = mproduct.category_id
INNER JOIN brand
ON brand.brand_id = mproduct.brand_id WHERE id = '$product_id'";
            ...
            <div class="row g-4">
                <div class="col-sm-12 col-xl-6">
                    <div class="bg-light rounded h-100 p-4">
                        ...
                        <img src="product_images/M_product/<?php echo $fetch_product['productImage'] ?>" alt="" width="150px">
...
<?php
    ...
    if (isset($_POST['update_product'])) {
        ...
        $path = 'product_images/M_product/' . $img_name;

        if (is_uploaded_file($img_tmpname)) {
            move_uploaded_file($img_tmpname, $path);

            $update = "UPDATE `mproduct` SET `productName`='$p_name',`productPrice`='$p_price',`productDescription`='$p_descrip', `productImage` = '$img_name', `category_id`='$c_id',`brand_id`='$b_id' WHERE `id` = '$product_id'";
...
if (isset($_GET['j'])) {
    ...
    $delete = "DELETE FROM mproduct WHERE id = '$product_id'";
